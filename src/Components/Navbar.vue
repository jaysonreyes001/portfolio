<template lang="">
     ...
     <nav class="fixed w-full z-20 top-0 start-0 border-b dark:border-white/5 border-black/10 dark:bg-black/30 bg-white/30 backdrop-blur-md">
          <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-5">
               ...
               <div class="inline-flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
                    ...
                    <button @click="handleMobileSideBar" type="button" class="inline-flex items-center p-2 w-9 h-9 justify-center text-sm text-body rounded md:hidden dark:hover:bg-primary-dark hover:bg-primary-light hover:text-heading focus:outline-none focus:ring-2 focus:ring-neutral-tertiary" aria-controls="navbar-cta" aria-expanded="false">
               ...
               <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-cta">
                    <ul class="text-md flex flex-col p-4 md:p-0 mt-4 border border-default md:flex-row md:space-x-8 md:mt-0 md:border-0 text-secondary-dark">
                         <li v-for="(item,index) in menu" :id="`${item.href}-nav`" @click.prevent="scrollToSection(item.href,$event)">
                              <a href="#" :class="activeMenu == item.href ? 'font-bold dark:text-primary-dark text-primary-light' :  'dark:text-white text-black' " class="transition font-semibold block py-2 px-3 text-heading rounded dark:hover:text-primary-dark hover:text-primary-light transition">
                                   {{item.name}}
                              </a>
                         </li>
                    </ul>
